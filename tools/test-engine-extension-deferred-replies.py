#!/usr/bin/env python3
"""Run native extension deferred reply lifetime checks with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = ('WebExtensionDeferredReply.h',)
...
def native():
    ...
    report = {'scope': 'native deferred reply lifetime helper only; no extension context, background page, IPC or JavaScript runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('EngineExtensionDeferredReplyTests.cpp',):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Deferred reply helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native deferred-reply test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    for name in ('test-engine-extension-deferred-replies.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-deferred-reply-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-deferred-reply-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native deferred reply helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-deferred-replies.py', '--native']),
