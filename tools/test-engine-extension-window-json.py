#!/usr/bin/env python3
"""Run the native window query parser and JavaScriptCore window/tab converters with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'Shared/Extensions': ('WebExtensionWindowQueryParser.h', 'WebExtensionWindowQueryParser.cpp'),
    'WebProcess/Extensions/Bindings': ('JSWebExtensionWindowParameters.h', 'JSWebExtensionWindowParameters.cpp', 'JSWebExtensionTabParameters.h', 'JSWebExtensionTabParameters.cpp', 'JSWebExtensionString.h', 'JSWebExtensionString.cpp'),
}
...
def native():
    ...
    report = {'scope': 'native structural window-option parsing and real JavaScriptCore window/tab conversion only; no WebExtension, context, controller or browser runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionWindowQueryParser.cpp', 'JSWebExtensionWindowParameters.cpp', 'JSWebExtensionTabParameters.cpp', 'JSWebExtensionString.cpp', 'EngineExtensionWindowJSONTests.cpp'):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native window JSON test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionWindowJSONTests.cpp'
    ...
    for name in ('test-engine-extension-window-json.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-window-json-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-window-json-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native window JSON helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-window-json.py', '--native']),
