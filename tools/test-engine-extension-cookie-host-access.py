#!/usr/bin/env python3
"""Run native cookie host-grant decisions with actual match-pattern code and frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    ...
    'WebExtensionCookieHostAccess.cpp': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionCookieHostAccess.cpp'),
    'WebExtensionCookieHostAccess.h': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionCookieHostAccess.h'),
}
...
def native():
    ...
    report = {'scope': 'native cookie host grant/denial/expiry evaluation with actual WebExtensionMatchPattern and UserContentURLPattern; no cookie API permission, private store, context lifecycle, IPC or browser runtime',
    ...
    objects = []
    for name in ('APIObject.cpp', 'UserContentURLPattern.cpp', 'LocalizedStrings.cpp', 'WebExtensionMatchPattern.cpp', 'WebExtensionCookieHostAccess.cpp', 'EngineExtensionCookieHostAccessTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Cookie host access helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native cookie host access test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionCookieHostAccessTests.cpp'
    ...
    for name in ('test-engine-extension-cookie-host-access.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-cookie-host-access-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-cookie-host-access-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native cookie host access stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-cookie-host-access.py', '--native']),
