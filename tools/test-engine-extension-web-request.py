...
HELPERS = {
    ...
    'WebExtensionWebRequestBody.h': Path('Source/WebKit/WebProcess/Extensions/WebExtensionWebRequestBody.h'),
    'WebExtensionWebRequestBody.cpp': Path('Source/WebKit/WebProcess/Extensions/WebExtensionWebRequestBody.cpp'),
    ...
    'WebExtensionWebRequestFilter.h': Path('Source/WebKit/WebProcess/Extensions/WebExtensionWebRequestFilter.h'),
    'WebExtensionWebRequestFilter.cpp': Path('Source/WebKit/WebProcess/Extensions/WebExtensionWebRequestFilter.cpp'),
    ...
    'ResourceLoadInfo.h': Path('Source/WebKit/Shared/ResourceLoadInfo.h'),
    'WebExtensionFrameIdentifier.cpp': Path('Source/WebKit/Shared/Extensions/WebExtensionFrameIdentifier.cpp'),
    'WebExtensionFrameIdentifier.h': Path('Source/WebKit/Shared/Extensions/WebExtensionFrameIdentifier.h'),
    'WebExtensionTabIdentifier.h': Path('Source/WebKit/Shared/Extensions/WebExtensionTabIdentifier.h'),
    'WebExtensionWindowIdentifier.h': Path('Source/WebKit/Shared/Extensions/WebExtensionWindowIdentifier.h'),
}
...
def native():
    ...
    report = {'scope': 'native request filters, resource metadata and upload-body conversion using actual WebKit types, WTF and JavaScriptCore; no listener, extension context, IPC or browser runtime',
    ...
    objects = []
    for name in ('APIObject.cpp', 'UserContentURLPattern.cpp', 'LocalizedStrings.cpp', 'WebExtensionMatchPattern.cpp', 'WebExtensionFrameIdentifier.cpp', 'WebExtensionWebRequestFilter.cpp', 'JSWebExtensionString.cpp', 'WebExtensionWebRequestBody.cpp', 'EngineExtensionWebRequestTests.cpp'):
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionWebRequestTests.cpp'
    ...
    for name in ('test-engine-extension-web-request.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-web-request-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-web-request-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native web request helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-web-request.py', '--native']),
