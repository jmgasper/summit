#!/usr/bin/env python3
"""Run native extension page-port ownership checks with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = ('WebExtensionPagePortMap.h',)
...
def native():
    ...
    report = {'scope': 'native page-port ownership helper only; no WebExtension, context, controller, IPC or JavaScript runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('EngineExtensionPagePortTests.cpp',):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Page-port helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native page-port test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    for name in ('test-engine-extension-page-ports.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-page-port-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-page-port-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native page-port helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-page-ports.py', '--native']),
