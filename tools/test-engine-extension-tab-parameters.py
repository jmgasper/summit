#!/usr/bin/env python3
"""Run native tab metadata conversion checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {name: Path('Source/WebKit/WebProcess/Extensions/Bindings') / name
           for name in ('JSWebExtensionTabParameters.h', 'JSWebExtensionTabParameters.cpp',
                        'JSWebExtensionString.h', 'JSWebExtensionString.cpp')}
HELPERS.update({name: Path('Source/WebKit/Shared/Extensions') / name
                for name in ('WebExtensionTabParameters.h', 'WebExtensionTabIdentifier.h',
                             'WebExtensionWindowIdentifier.h')})
...
def native():
    ...
    report = {'scope': 'native tab metadata conversion only; no extension context, browser tab, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('JSWebExtensionTabParameters.cpp', 'JSWebExtensionString.cpp', 'EngineExtensionTabParameterTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Tab metadata helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native tab metadata test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionTabParameterTests.cpp'
    ...
    for name in ('test-engine-extension-tab-parameters.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-tab-parameter-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-tab-parameter-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native tab metadata helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-tab-parameters.py', '--native']),
