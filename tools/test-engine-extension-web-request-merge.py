#!/usr/bin/env python3
"""Compile and run the real native webRequest blocking-reply merger with frozen WTF.

No WebCore/WebKit objects are linked. This does not validate the controller,
extension API, events, permissions or network interception. Run runtime tests
...
PREFIX = '/boot/home/summit/extension-web-request-merge-tests.'
SCRIPT = 'test-engine-extension-web-request-merge.py'
HELPERS = {
    'Shared/Extensions': ('WebExtensionWebRequestDecisionHaiku.h', 'WebExtensionWebRequestDecisionHaiku.cpp', 'WebExtensionWebRequestMergeHaiku.h', 'WebExtensionWebRequestMergeHaiku.cpp'),
}
UNITS = ('WebExtensionWebRequestDecisionHaiku.cpp', 'WebExtensionWebRequestMergeHaiku.cpp', 'EngineExtensionWebRequestMergeTests.cpp')
LIBRARIES = ('libJavaScriptCore.so.18.7.4', 'libicudata.so.78.3', 'libicui18n.so.78.3', 'libicuuc.so.78.3')
SCOPE = 'native blocking webRequest reply arbitration and header deltas using retained installation/listener metadata, header visibility and DNR constraints; no live permission checks, extension callback invocation, IPC, request suspension or network mutation'
...
def native(compile_only=False, run_only=False):
    ...
    if run_only:
        ...
        for name in UNITS:
            ...
            command = ['c++', *flags, '-MMD', '-MF', str(dependency), '-c', str(output / name), '-o', str(obj)]
            ...
            if not result.returncode:
                ...
                for filename in shlex.split(dependencies):
                    path = Path(filename) if Path(filename).is_absolute() else build / filename
                    report['native_inputs'][str(path)] = digest(path)
        ...
        if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
            raise RuntimeError('WebRequest merge helper unexpectedly links WebCore or WebKit')
    ...
    if compile_only:
        result_path.write_text(json.dumps(report, indent=2) + '\n')
        print('Native webRequest merge model compiled and linked; runtime not executed.', flush=True)
        return 0
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native webRequest merge model test timed out and was terminated; failed run'
    report['native_crash_log'] = crash_log.finish()
    verify_files(report['native_inputs'])
    verify_files(report['artifacts'])
    verify_files(staged)
    verify_files(libraries)
    marker = re.findall(r'^WEB_REQUEST_MERGE_RESULT PASS checks=(\d+) failures=0$', report['output'], re.M)
    report['checks'] = int(marker[0]) if len(marker) == 1 else 0
    report['passed'] = report['exit'] == 0 and report['checks'] == 58 and report['native_crash_log']['passed']
    result_path.write_text(json.dumps(report, indent=2) + '\n')
...
def host(overlay, bundle_result, compile_only, resume):
    ...
    if resume:
        ...
        if not re.fullmatch(re.escape(PREFIX) + r'[A-Za-z0-9]+', stage):
            raise RuntimeError('Invalid native webRequest merge test stage')
        ...
        test = ROOT / 'tests/EngineExtensionWebRequestMergeTests.cpp'
    ...
    print('Native webRequest merge stage: ' + stage, flush=True)
