#!/usr/bin/env python3
"""Run native registered script JSON parsing checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {name: Path('Source/WebKit/Shared/Extensions') / name
           for name in ('WebExtensionRegisteredScriptParser.h', 'WebExtensionRegisteredScriptParser.cpp',
                        'WebExtensionRegisteredScriptParameters.h')}
...
def native():
    ...
    report = {'scope': 'native registered script JSON parsing only; no extension context, browser tab, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionRegisteredScriptParser.cpp', 'EngineExtensionRegisteredScriptTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Registered script helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native registered script test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionRegisteredScriptTests.cpp'
    ...
    for name in ('test-engine-extension-registered-scripts.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-registered-script-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-registered-script-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native registered script helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-registered-scripts.py', '--native']),
