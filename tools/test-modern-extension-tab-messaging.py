#!/usr/bin/env python3
"""Verify installed tab messaging across tabs, frames, documents, and extensions."""
import argparse
import hashlib
...
def native(args):
    ...
    executable = ROOT / 'ModernExtensionTabMessagingTests'
    source = bundle / 'source'
    command = ['c++', '-std=c++23', '-O2', '-Wall', '-Wextra', '-Wno-multichar',
               '-I' + str(source / 'src'), '-I' + str(source / 'vendor'),
               str(ROOT / 'tests/ModernExtensionTabMessagingTests.cpp'), '-lbe', '-o', str(executable)]
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            elif self.path == '/' + token + '/isolation-ready':
                with report_lock:
                    ready = {event.get('label') for event in isolation if event.get('kind') == 'ready'} == {'a/main', 'a/child', 'b/main', 'b/child'}
                self.respond(json.dumps({'ready': ready}).encode())
            elif self.path.startswith('/' + token + '/target/'):
                suffix = self.path.split('/target/', 1)[1]
                body = '<!doctype html><meta charset="utf-8"><title>Summit tab messaging</title><h1>Tab messaging: ' + suffix + '</h1>'
        ...
        def do_POST(self):
            prefix = '/' + token + '/report/'
            is_isolation = self.path == '/' + token + '/isolation'
            phase = self.path[len(prefix):] if self.path.startswith(prefix) else ''
            if not is_isolation and phase not in ('initial', 'navigated', 'complete', 'failure'):
            ...
            try:
                ...
                with report_lock:
                    if is_isolation:
                        ...
                        (control / 'isolation.json').write_text(json.dumps(isolation) + '\n')
                    else:
    ...
    other = ROOT / 'installed-other-package'
    shutil.copytree(ROOT / 'other-package', other)
    config = other / 'config.js'
    if config.read_text().count('__CONTROL_JSON__') != 1:
        raise RuntimeError('Missing other control URL placeholder')
    config.write_text(config.read_text().replace('__CONTROL_JSON__', json.dumps(control_url)))
    package_inputs = {str(path.relative_to(ROOT)): digest(path) for folder in (package, other)
    ...
    try:
        with (ROOT / 'browser.log').open('w') as browser_log, (ROOT / 'harness.log').open('w') as harness_log:
            ...
            save()
            try:
                command = [str(executable), str(process.pid), str(bundle / 'Summit'), str(package), str(control), target_url, str(other)]
        ...
        report['passed'] = (report.get('harness_exit') == 0 and report.get('browser_exit') == 0
            and report['group_drained'] and not report['forced_cleanup'] and 'EXTENSION_TAB_MESSAGING_RESULT PASS checks=' in report['output'])
        ...
        report['phases'] = {name: json.loads((control / (name + '.json')).read_text())
                            for name in ('initial', 'navigated', 'complete', 'failure') if (control / (name + '.json')).exists()}
        report['server_errors'] = server_errors
        report['isolation_events'] = isolation
        report['other_extension_listeners_ready'] = {event.get('label') for event in isolation if event.get('kind') == 'ready' and event.get('id') == 'summit-tab-messaging-other'} == {'a/main','a/child','b/main','b/child'}
        report['no_cross_extension_messages_observed'] = all(event.get('kind') == 'ready' for event in isolation)
        report['extension_isolation_verified'] = (report.get('harness_exit') == 0
            and report['other_extension_listeners_ready'] and report['no_cross_extension_messages_observed'])
...
def host(args):
    ...
    names = ('tests/ModernExtensionTabMessagingTests.cpp', 'tests/ModernExtensionOverflowTests.cpp',
    ...
    for name, target in (('tab-messaging', 'package'), ('tab-messaging-other', 'other-package')):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-tab-messaging-browser.XXXXXXXX', capture_output=True, text=True, check=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-tab-messaging-browser.') or not stage.rsplit('.', 1)[-1].isalnum():
        raise RuntimeError('Invalid native stage')
    output = ROOT / '.vm' / ('modern-extension-tab-messaging-' + secrets.token_hex(12))
    output.mkdir()
