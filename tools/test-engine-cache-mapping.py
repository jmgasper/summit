#!/usr/bin/env python3
"""Run native network cache mapping and read-only descriptor transport checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {name: Path('Source/WebKit/NetworkProcess/cache') / name
           for name in ('NetworkCacheData.h', 'NetworkCacheData.cpp', 'NetworkCacheDataCurl.cpp')}
HELPERS.update({name: Path('Source/WebCore/platform') / name for name in ('SharedMemory.cpp',)})
HELPERS['HaikuProcess.h'] = Path('Source/WebKit/UIProcess/Launcher/haiku/HaikuProcess.h')
HELPERS['SharedMemoryHaiku.cpp'] = Path('Source/WebCore/platform/haiku/SharedMemoryHaiku.cpp')
HELPERS['APIContentRuleListStoreHaiku.cpp'] = Path('Source/WebKit/UIProcess/API/haiku/APIContentRuleListStoreHaiku.cpp')
...
def native(regression=False):
    ...
    watched = [build / 'build.ninja', build / 'cmakeconfig.h', ENGINE / '.summit-source-manifest.json',
               ENGINE / 'Source/WebKit/WebKitPrefix.h', ENGINE / 'Source/WebCore/platform/SharedMemory.h']
    ...
    (output / 'cmakeconfig.h').write_text(config.replace('#define ENABLE_CONTENT_EXTENSIONS 0', '#define ENABLE_CONTENT_EXTENSIONS 1'))
    fields = {}
    ...
    flags = ['-I' + str(output), '-iquote', str(output), *flags,
             '-include', str(watched[3]), '-I' + str(ENGINE / 'Source/WebCore'), '-iquote', str(build / 'WebCore/PrivateHeaders/WebCore'), '-I' + str(ENGINE / 'Source/WebKit/UIProcess/Launcher/haiku'), '-DWEBCORE_EXPORT=', '-ffunction-sections', '-fdata-sections', '-fvisibility=hidden', '-fdiagnostics-color=never', '-fmax-errors=5']
    report = {'scope': 'native network cache mapping and read-only descriptor transport only; actual SharedMemory and NetworkCache Data units; no WebCore/WebKit libraries, IPC transport or rule execution',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False, 'shared_memory_regression': regression}
    objects = []
    units = ('SharedMemory.cpp', 'SharedMemoryHaiku.cpp', 'EngineSharedMemoryTests.cpp') if regression else ('NetworkCacheData.cpp', 'NetworkCacheDataCurl.cpp', 'SharedMemory.cpp', 'SharedMemoryHaiku.cpp', 'APIContentRuleListStoreHaiku.cpp', 'EngineNetworkCacheMappingTests.cpp')
    for name in units:
        ...
        if name != 'APIContentRuleListStoreHaiku.cpp':
            objects.append(obj)
    ...
    report['link_exit'] = link.returncode
    report['link_output'] = link.stdout
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Cache mapping helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native cache mapping test timed out and was terminated; failed run'
...
def host(overlay, regression=False):
    files = {}
    ...
    test = ROOT / 'tests' / ('EngineSharedMemoryTests.cpp' if regression else 'EngineNetworkCacheMappingTests.cpp')
    ...
    for name in ('test-engine-cache-mapping.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/cache-mapping-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/cache-mapping-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native cache mapping helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-cache-mapping.py', '--native', *(['--shared-memory-regression'] if regression else [])]),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--shared-memory-regression', action='store_true', help='Run the existing native shared-memory regression suite')
    parser.add_argument('--overlay', help='Candidate source root containing Source/')
    args = parser.parse_args()
    raise SystemExit(native(args.shared_memory_regression) if args.native else host(args.overlay, args.shared_memory_regression))
