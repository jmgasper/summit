#!/usr/bin/env python3
"""Run native extension test-message queue checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'WebExtensionTestMessageQueueHaiku.h': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionTestMessageQueueHaiku.h'),
...
def native():
    ...
    report = {'scope': 'native queue ownership, bounded draining and weak-registry availability only; test frame objects, no extension context, background page or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('EngineExtensionTestMessageQueueTests.cpp',):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native test-message queue test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionTestMessageQueueTests.cpp'
    ...
    for name in ('test-engine-extension-test-message-queue.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-test-message-queue-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-test-message-queue-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native test-message queue stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-test-message-queue.py', '--native']),
