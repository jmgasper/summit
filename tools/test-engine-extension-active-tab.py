#!/usr/bin/env python3
"""Run native activeTab origin grants and revocation with actual match-pattern code and frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    ...
    'WebExtensionActiveTabGrant.cpp': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionActiveTabGrant.cpp'),
    'WebExtensionActiveTabGrant.h': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionActiveTabGrant.h'),
}
...
def native():
    ...
    report = {'scope': 'native activeTab origin scoping, file gating and revocation with actual WebExtensionMatchPattern and URL; no context lifecycle, permissions API, native input, IPC or browser runtime',
    ...
    objects = []
    for name in ('APIObject.cpp', 'UserContentURLPattern.cpp', 'LocalizedStrings.cpp', 'WebExtensionMatchPattern.cpp', 'WebExtensionActiveTabGrant.cpp', 'EngineExtensionActiveTabTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('ActiveTab grant helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native activeTab grant test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionActiveTabTests.cpp'
    ...
    for name in ('test-engine-extension-active-tab.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-active-tab-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-active-tab-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native activeTab grant stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-active-tab.py', '--native']),
