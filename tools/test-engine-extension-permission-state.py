#!/usr/bin/env python3
"""Run the native permission state and atomic JSON-file helpers with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    ...
    'Shared/Extensions': ('WebExtensionPermissionState.h', 'WebExtensionPermissionState.cpp'),
}
...
def native():
    ...
    report = {'scope': 'native permission-details parser, saved-grant codec and atomic state-file writer only; no WebExtension, context, controller or browser runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionStateHaiku.cpp', 'WebExtensionPermissionState.cpp', 'EngineExtensionPermissionStateTests.cpp'):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native permission state test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionPermissionStateTests.cpp'
    ...
    for name in ('test-engine-extension-permission-state.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-permission-state-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-permission-state-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native permission state helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-permission-state.py', '--native']),
