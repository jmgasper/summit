#!/usr/bin/env python3
"""Run native command shortcut and settings checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {name: Path('Source/WebKit/Shared/Extensions') / name
           for name in ('WebExtensionCommandShortcut.h', 'WebExtensionCommandShortcut.cpp',
                        'WebExtensionCommandSettings.h', 'WebExtensionCommandSettings.cpp')}
...
def native():
    ...
    report = {'scope': 'native command shortcut and settings only; no extension context, disk writes, keyboard routing, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionCommandShortcut.cpp', 'WebExtensionCommandSettings.cpp', 'EngineExtensionCommandSettingsTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Command settings helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native command settings test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionCommandSettingsTests.cpp'
    ...
    for name in ('test-engine-extension-command-settings.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-command-settings-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-command-settings-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native command settings helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-command-settings.py', '--native']),
