#!/usr/bin/env python3
"""Run native extension message-queue checks with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = {'WebExtensionNativePortMessageQueue.h': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionNativePortMessageQueue.h'),
           'WebExtensionConstants.h': Path('Source/WebKit/Shared/Extensions/WebExtensionConstants.h')}
...
def native():
    ...
    report = {'scope': 'native port message queue helper only; no WebExtension, context, controller, IPC or JavaScript runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('EngineExtensionNativePortQueueTests.cpp',):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Native port queue helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native port queue test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionNativePortQueueTests.cpp'
    ...
    for name in ('test-engine-extension-native-port-queue.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-native-port-queue-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-native-port-queue-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native port queue helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-native-port-queue.py', '--native']),
