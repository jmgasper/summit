#!/usr/bin/env python3
"""Run native extension promise checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'JSWebExtensionPromise.h': Path('Source/WebKit/WebProcess/Extensions/Bindings/JSWebExtensionPromise.h'),
    'JSWebExtensionPromise.cpp': Path('Source/WebKit/WebProcess/Extensions/Bindings/JSWebExtensionPromise.cpp'),
}
...
def native():
    ...
    report = {'scope': 'native intrinsic promise transformation and capture lifetime only; no WebExtensionAPITest, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('JSWebExtensionPromise.cpp', 'EngineExtensionPromiseTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Promise helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native promise test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionPromiseTests.cpp'
    ...
    for name in ('test-engine-extension-promises.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-promise-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-promise-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native promise stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-promises.py', '--native']),
