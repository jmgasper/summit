#!/usr/bin/env python3
"""Run native extension browser tab registry checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    name: Path('Source/WebKit/UIProcess/haiku') / name
    for name in ('BrowserTabRegistryHaiku.h', 'BrowserTabRegistryHaiku.cpp')
}
...
def native():
    ...
    report = {'scope': 'native browser tab metadata registry using real BMessengers and generated page identifiers; no WebPage, browser SDK, extension context or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('BrowserTabRegistryHaiku.cpp', 'EngineExtensionBrowserTabRegistryTests.cpp'):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native browser tab registry test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionBrowserTabRegistryTests.cpp'
    ...
    for name in ('test-engine-browser-tab-registry.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-browser-tab-registry-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-browser-tab-registry-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native browser tab registry stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-browser-tab-registry.py', '--native']),
