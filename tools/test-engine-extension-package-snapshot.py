#!/usr/bin/env python3
"""Run native package snapshot and SHA-256 resource identity checks."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    ...
    'pal/crypto/CryptoTypes.h': Path('Source/WebCore/PAL/pal/crypto/CryptoTypes.h'),
    'CryptoDigest.h': Path('Source/WebCore/PAL/pal/crypto/CryptoDigest.h'),
    'CryptoDigestOpenSSL.cpp': Path('Source/WebCore/PAL/pal/crypto/openssl/CryptoDigestOpenSSL.cpp'),
}
...
def native():
    ...
    libraries.append(Path('/boot/system/lib/libcrypto.so.3'))
    ...
    flags = ['-I' + str(output), '-iquote', str(output), *flags,
             '-include', str(watched[3]), '-ffunction-sections', '-fdata-sections', '-fdiagnostics-color=never', '-fmax-errors=5']
    report = {'scope': 'native package snapshots and resource fingerprints using actual PAL SHA-256 and WTF; no WebExtension constructor, context or browser runtime',
    ...
    subprocess.run(['c++', *map(str, objects), str(libraries[0]), '-lcrypto', '-lbe', '-lnetwork',
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Package snapshot helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Package snapshot test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    for name in ('test-engine-extension-package-snapshot.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-package-snapshot-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-package-snapshot-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Package snapshot helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-package-snapshot.py', '--native']),
