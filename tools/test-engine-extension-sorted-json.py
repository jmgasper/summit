#!/usr/bin/env python3
"""Run native sorted JSON serialization checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
def native():
    ...
    report = {'scope': 'native sorted JSON serialization only; no extension context, browser tab, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('JSWebExtensionWrapper.cpp', 'JSWebExtensionString.cpp', 'EngineExtensionSortedJSONTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Sorted JSON helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native sorted JSON test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionSortedJSONTests.cpp'
    ...
    for name in ('test-engine-extension-sorted-json.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-sorted-json-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-sorted-json-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native sorted JSON helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-sorted-json.py', '--native']),
