#!/usr/bin/env python3
"""Run native background listener persistence checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {'WebExtensionBackgroundListenersHaiku.h': Path('Source/WebKit/UIProcess/Extensions/haiku/WebExtensionBackgroundListenersHaiku.h'),
           'WebExtensionEventListenerType.h': Path('Source/WebKit/Shared/Extensions/WebExtensionEventListenerType.h'),
           'WebExtensionContentWorldType.h': Path('Source/WebKit/Shared/Extensions/WebExtensionContentWorldType.h')}
...
def native(enable_dnr_event=False):
    ...
    config = config.replace('#define ENABLE_WK_WEB_EXTENSIONS 0', '#define ENABLE_WK_WEB_EXTENSIONS 1')
    config += '\n#undef ENABLE_DNR_ON_RULE_MATCHED_DEBUG\n#define ENABLE_DNR_ON_RULE_MATCHED_DEBUG ' + str(int(enable_dnr_event)) + '\n'
    isolated_config = output / 'cmakeconfig.h'
    isolated_config.write_text(config)
    isolated_config_sha256 = digest(isolated_config)
    fields = {}
    ...
    report = {'scope': 'native background listener persistence only; no extension context, background page, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'dnr_event_enabled_for_probe': enable_dnr_event, 'isolated_cmakeconfig_sha256': isolated_config_sha256,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('EngineExtensionBackgroundListenerTests.cpp',):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Background listener helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native background listener test timed out and was terminated; failed run'
    report['native_crash_log'] = crash_log.finish()
    report['isolated_configuration_unchanged'] = digest(isolated_config) == isolated_config_sha256
    ...
    report['passed'] = (report['exit'] == 0 and report['native_crash_log']['passed'] and report['isolated_configuration_unchanged'] and report['inputs_unchanged']
...
def host(overlay, enable_dnr_event=False):
    files = {}
    ...
    test = ROOT / 'tests/EngineExtensionBackgroundListenerTests.cpp'
    ...
    for name in ('test-engine-extension-background-listeners.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-background-listener-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-background-listener-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native background listener helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-background-listeners.py', '--native'] + (['--enable-dnr-event'] if enable_dnr_event else [])),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--enable-dnr-event', action='store_true', help='Test the alternate feature-gated event enum layout')
    args = parser.parse_args()
    raise SystemExit(native(args.enable_dnr_event) if args.native else host(args.overlay, args.enable_dnr_event))
