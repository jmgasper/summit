#!/usr/bin/env python3
"""Compile and run the real native webRequest blocking-reply decoder with frozen WTF.
...
PREFIX = '/boot/home/summit/extension-web-request-decisions-tests.'
SCRIPT = 'test-engine-extension-web-request-decisions.py'
HELPERS = {
    'Shared/Extensions': ('WebExtensionWebRequestDecisionHaiku.h', 'WebExtensionWebRequestDecisionHaiku.cpp'),
}
UNITS = ('WebExtensionWebRequestDecisionHaiku.cpp', 'EngineExtensionWebRequestDecisionTests.cpp')
LIBRARIES = ('libJavaScriptCore.so.18.7.4', 'libicudata.so.78.3', 'libicui18n.so.78.3', 'libicuuc.so.78.3')
SCOPE = 'native blocking webRequest reply decoding only; no permissions, callback invocation, IPC, request suspension or network mutation'
...
def native(compile_only=False, run_only=False):
    ...
    if run_only:
        ...
        if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
            raise RuntimeError('WebRequest decision helper unexpectedly links WebCore or WebKit')
    ...
    if compile_only:
        result_path.write_text(json.dumps(report, indent=2) + '\n')
        print('Native webRequest decision model compiled and linked; runtime not executed.', flush=True)
        return 0
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native webRequest decision model test timed out and was terminated; failed run'
    report['native_crash_log'] = crash_log.finish()
    verify_files(report['native_inputs'])
    verify_files(report['artifacts'])
    verify_files(staged)
    verify_files(libraries)
    marker = re.findall(r'^WEB_REQUEST_DECISION_RESULT PASS checks=(\d+) failures=0$', report['output'], re.M)
    report['checks'] = int(marker[0]) if len(marker) == 1 else 0
    report['passed'] = report['exit'] == 0 and report['checks'] == 62 and report['native_crash_log']['passed']
    result_path.write_text(json.dumps(report, indent=2) + '\n')
...
def host(overlay, bundle_result, compile_only, resume):
    ...
    if resume:
        ...
        if not re.fullmatch(re.escape(PREFIX) + r'[A-Za-z0-9]+', stage):
            raise RuntimeError('Invalid native webRequest decision test stage')
        ...
        test = ROOT / 'tests/EngineExtensionWebRequestDecisionTests.cpp'
    ...
    print('Native webRequest decision stage: ' + stage, flush=True)
