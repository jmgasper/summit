#!/usr/bin/env python3
"""Verify i18n through native installation, background, popup, and content scripts."""
import argparse
import hashlib
...
def native(args):
    ...
    executable = ROOT / 'ModernExtensionLocalizationTests'
    source = bundle / 'source'
    command = ['c++', '-std=c++23', '-O2', '-Wall', '-Wextra', '-Wno-multichar',
               '-I' + str(source / 'src'), '-I' + str(source / 'vendor'),
               str(ROOT / 'tests/ModernExtensionLocalizationTests.cpp'), '-lbe', '-o', str(executable)]
    ...
    shutil.copyfile(ROOT / 'unlocalized-package/expected.json', control / 'unlocalized-expected.json')
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_OPTIONS(self):
            ...
            self.end_headers()
        def do_GET(self):
            if self.path == '/' + token + '/accept':
                self.respond(json.dumps({'acceptLanguage': self.headers.get('Accept-Language', '')}).encode())
            elif self.path in ('/' + token + '/target/setup', '/' + token + '/target/test'):
                self.respond(b'<!doctype html><meta charset="utf-8"><title>Summit localization test</title><h1>Extension localization</h1><p>The installed content script adds its localized message here.</p>', 'text/html; charset=utf-8')
            else: self.send_error(404)
        def do_POST(self):
            prefix = '/' + token + '/report/'
            if not self.path.startswith(prefix) or self.path[len(prefix):] not in ('background', 'popup', 'content', 'unlocalized'):
            ...
            try:
                ...
                if not isinstance(value, dict) or value.get('world') != world:
                    raise ValueError('Incorrect report world')
                with report_lock:
                    destination = control / (world + '.json')
                    if destination.exists():
                        raise ValueError('Duplicate ' + world + ' report')
                    temporary = control / (world + '.tmp')
    ...
    unlocalized = ROOT / 'installed-unlocalized-package'
    shutil.copytree(ROOT / 'unlocalized-package', unlocalized)
    config = unlocalized / 'config.js'
    if config.read_text().count('__CONTROL_JSON__') != 1:
        raise RuntimeError('Missing unlocalized control URL placeholder')
    config.write_text(config.read_text().replace('__CONTROL_JSON__', json.dumps(control_url)))
    package_inputs = {str(path.relative_to(ROOT)): digest(path) for folder in (package, unlocalized)
    ...
    try:
        with (ROOT / 'browser.log').open('w') as browser_log, (ROOT / 'harness.log').open('w') as harness_log:
            ...
            save()
            try:
                command = [str(executable), str(process.pid), str(bundle / 'Summit'), str(package), str(control), target_url, str(unlocalized)]
        ...
        report['passed'] = (report.get('harness_exit') == 0 and report.get('browser_exit') == 0
            and report['group_drained'] and not report['forced_cleanup'] and 'EXTENSION_LOCALIZATION_RESULT PASS checks=' in report['output'])
        ...
        server.shutdown()
        server.server_close()
        report['worlds'] = {name: json.loads((control / (name + '.json')).read_text())
                            for name in ('background', 'popup', 'content', 'unlocalized') if (control / (name + '.json')).exists()}
...
def host(args):
    ...
    names = ('tests/ModernExtensionLocalizationTests.cpp', 'tests/ModernExtensionOverflowTests.cpp',
    ...
    for name, target in (('i18n', 'package'), ('i18n-unlocalized', 'unlocalized-package')):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-localization-browser.XXXXXXXX', capture_output=True, text=True, check=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-localization-browser.') or not stage.rsplit('.', 1)[-1].isalnum():
        raise RuntimeError('Invalid native stage')
    output = ROOT / '.vm' / ('modern-extension-localization-' + secrets.token_hex(12))
    output.mkdir()
