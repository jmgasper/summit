#!/usr/bin/env python3
"""Run native native rejected promise construction checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {name: Path('Source/WebKit/Shared/Extensions') / name
           for name in ('WebExtensionUtilities.h', 'WebExtensionUtilities.cpp')}
HELPERS.update({name: Path('Source/WebKit/WebProcess/Extensions/Bindings') / name
                for name in ('JSWebExtensionWrapper.h', 'JSWebExtensionWrapper.cpp', 'JSWebExtensionString.h', 'JSWebExtensionString.cpp')})
...
def native():
    ...
    report = {'scope': 'native native rejected promise construction only; no extension context, browser tab, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionUtilities.cpp', 'JSWebExtensionWrapper.cpp', 'JSWebExtensionString.cpp', 'EngineExtensionRejectedPromiseTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Rejected promise helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native rejected promise test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionRejectedPromiseTests.cpp'
    ...
    for name in ('test-engine-extension-rejected-promises.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-rejected-promise-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-rejected-promise-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native rejected promise helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-rejected-promises.py', '--native']),
