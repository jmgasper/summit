#!/usr/bin/env python3
"""Run native extension website-data selection checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'WebExtensionWebsiteData.h': Path('Source/WebKit/UIProcess/Extensions/WebExtensionWebsiteData.h'),
    'WebExtensionWebsiteData.cpp': Path('Source/WebKit/UIProcess/Extensions/WebExtensionWebsiteData.cpp'),
    'WebExtensionMatchPattern.cpp': Path('Source/WebKit/UIProcess/Extensions/WebExtensionMatchPattern.cpp'),
    'SecurityOriginData.cpp': Path('Source/WebCore/page/SecurityOriginData.cpp'),
}
...
def native():
    ...
    watched += [ENGINE / path for path in (
    ...
    for flag in raw:
        if flag == '-include':
            ...
            flags.append(flag)
    # Haiku executables link as shared ELF images. Hide WebCore's explicit exports
    # in this isolated harness so unused origin functions can be garbage-collected.
    # PlatformExportMacros.h expressly supports a predefined WEBCORE_EXPORT.
    flags = ['-I' + str(output), '-idirafter', str(build / 'WebCore/PrivateHeaders/WebCore'), '-iquote', str(output), *flags,
             '-DWEBCORE_EXPORT=',
             '-include', str(watched[3]), '-ffunction-sections', '-fdata-sections', '-fvisibility=hidden', '-fdiagnostics-color=never', '-fmax-errors=5']
    report = {'scope': 'native origin, extension-scheme and record-selection helpers; no website-data store, extension context, WebKit library or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False,
              'isolated_webcore_exports_hidden': True, 'compile_flags': flags}
    objects = []
    for name in ('WebExtensionMatchPattern.cpp', 'SecurityOriginData.cpp', 'WebExtensionWebsiteData.cpp', 'EngineExtensionWebsiteDataTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Website-data selection helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['output'] = partial_output + '\nNative website-data selection test timed out and was terminated; failed run\n'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionWebsiteDataTests.cpp'
    ...
    for name in ('test-engine-extension-website-data.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-website-data-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-website-data-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native website-data selection helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-website-data.py', '--native']),
