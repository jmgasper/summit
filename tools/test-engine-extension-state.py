#!/usr/bin/env python3
"""Run the native JSON state-file helper with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = ('WebExtensionStateHaiku.h', 'WebExtensionStateHaiku.cpp')
...
def native():
    ...
    objects = []
    for name in ('WebExtensionStateHaiku.cpp', 'EngineExtensionStateTests.cpp'):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native state test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionStateTests.cpp'
    ...
    for name in ('test-engine-extension-state.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-state-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-state-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native state helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-state.py', '--native']),
