#!/usr/bin/env python3
"""Exercise native consent broker and Summit windows using frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'ExtensionPermissionPromptHaiku.h': Path('Source/WebKit/UIProcess/haiku/ExtensionPermissionPromptHaiku.h'),
    'ExtensionPermissionPromptHaiku.cpp': Path('Source/WebKit/UIProcess/haiku/ExtensionPermissionPromptHaiku.cpp'),
    'WebKitExtensionPermission.h': Path('Source/WebKit/UIProcess/API/haiku/WebKitExtensionPermission.h'),
    'WebKit/WebKitExtensionPermission.h': Path('Source/WebKit/UIProcess/API/haiku/WebKitExtensionPermission.h'),
}
...
def native():
    ...
    report = {'scope': 'native consent broker and actual Summit permission windows; no loaded extension, permissions API, persisted grants, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('ExtensionPermissionPromptHaiku.cpp', 'ExtensionPermissionPrompt.cpp', 'EngineExtensionPermissionPromptTests.cpp'):
        obj = output / (Path(name).stem + '.o')
        unit_flags = ['-std=gnu++23', '-DSUMMIT_MODERN_WEBKIT=1', '-I' + str(output), '-Wall', '-Wextra', '-Wno-multichar'] if name == 'ExtensionPermissionPrompt.cpp' else [*flags, '-DSUMMIT_MODERN_WEBKIT=1']
        command = ['c++', *unit_flags, '-c', str(output / name), '-o', str(obj)]
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('Permission prompt helper unexpectedly links WebCore or WebKit')
    ...
    def capture():
        ready = output / 'permission-dialog.ready'
        while not stop_capture.wait(0.05):
            if ready.exists():
                shot = subprocess.run(['/bin/screenshot', '-s', str(output / 'permission-dialog.png')],
                    text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=10)
                screenshot.update({'exit': shot.returncode, 'output': shot.stdout,
                    'path': str(output / 'permission-dialog.png')})
                return
    capture_thread = threading.Thread(target=capture, daemon=True)
    crash_log = NativeCrashLog()
    capture_thread.start()
    try:
        result = subprocess.run([str(executable)], env=environment, cwd=output, timeout=40, text=True,
                                stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        report['exit'] = result.returncode
        report['output'] = result.stdout
    except subprocess.TimeoutExpired:
        report['exit'] = None
        report['output'] = 'Native permission prompt test timed out and was terminated; failed run'
    stop_capture.set()
...
def host(overlay):
    ...
    for name in ('ExtensionPermissionPrompt.h', 'ExtensionPermissionPrompt.cpp'):
        path = ROOT / 'src/ui' / name
        files[name] = (path, path.read_bytes())
    test = ROOT / 'tests/EngineExtensionPermissionPromptTests.cpp'
    ...
    for name in ('test-engine-extension-permission-prompts.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-permission-prompt-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-permission-prompt-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native permission prompt helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-permission-prompts.py', '--native']),
    ...
    image = remote('cat ' + shlex.quote(stage + '/sources/permission-dialog.png'), capture_output=True)
    if image.returncode == 0:
        (output / 'permission-dialog.png').write_bytes(image.stdout)
