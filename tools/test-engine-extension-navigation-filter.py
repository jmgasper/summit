...
HELPERS = {
    name: Path('Source/WebKit/WebProcess/Extensions') / name
    for name in ('WebExtensionNavigationURLFilter.h', 'WebExtensionNavigationURLFilter.cpp',
                 'WebExtensionNavigationParameters.h', 'WebExtensionNavigationParameters.cpp')
}
HELPERS['WebExtensionFrameIdentifier.cpp'] = Path('Source/WebKit/Shared/Extensions/WebExtensionFrameIdentifier.cpp')
HELPERS['WebExtensionFrameIdentifier.h'] = Path('Source/WebKit/Shared/Extensions/WebExtensionFrameIdentifier.h')
HELPERS['WebExtensionFrameParameters.h'] = Path('Source/WebKit/Shared/Extensions/WebExtensionFrameParameters.h')
HELPERS['WebExtensionTabIdentifier.h'] = Path('Source/WebKit/Shared/Extensions/WebExtensionTabIdentifier.h')
...
def native():
    ...
    report = {'scope': 'native URL filters, frame identifiers and navigation parameter serialization using actual WTF and JavaScriptCore; no listener, extension context, IPC or browser runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionNavigationURLFilter.cpp', 'WebExtensionFrameIdentifier.cpp', 'WebExtensionNavigationParameters.cpp', 'EngineExtensionNavigationFilterTests.cpp'):
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionNavigationFilterTests.cpp'
    ...
    for name in ('test-engine-extension-navigation-filter.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-navigation-filter-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-navigation-filter-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native navigation URL filter stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-navigation-filter.py', '--native']),
