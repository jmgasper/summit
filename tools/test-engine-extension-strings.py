#!/usr/bin/env python3
"""Run native UTF-16 string bridge checks with frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
HELPERS = ('JSWebExtensionString.h', 'JSWebExtensionString.cpp')
...
def native():
    ...
    report = {'scope': 'native UTF-16 bridge only; no extension context, controller, WebCore, WebKit or IPC runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('JSWebExtensionString.cpp', 'EngineExtensionStringTests.cpp'):
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('String helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native string test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionStringTests.cpp'
    ...
    for name in ('test-engine-extension-strings.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-string-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-string-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native string helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-strings.py', '--native']),
