#!/usr/bin/env python3
"""Run the native action parser, raw JavaScript tab IDs and existing dictionary conversion with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'Shared/Extensions': ('WebExtensionActionDetails.h', 'WebExtensionActionDetails.cpp'),
    'WebProcess/Extensions/Bindings': ('JSWebExtensionActionDetails.h', 'JSWebExtensionWrapper.h', 'JSWebExtensionWrapper.cpp', 'JSWebExtensionString.h', 'JSWebExtensionString.cpp'),
}
...
def native():
    ...
    report = {'scope': 'native structural action-details parsing, real JavaScriptCore raw tab-ID validation and existing JSON dictionary bridge only; no WebExtension, context, controller or browser runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionActionDetails.cpp', 'JSWebExtensionWrapper.cpp', 'JSWebExtensionString.cpp', 'EngineExtensionActionDetailsTests.cpp'):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native action details test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionActionDetailsTests.cpp'
    ...
    for name in ('test-engine-extension-action-details.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-action-details-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-action-details-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native action details helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-action-details.py', '--native']),
