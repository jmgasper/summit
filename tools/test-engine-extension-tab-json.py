#!/usr/bin/env python3
"""Run the native tab query parser and existing JavaScriptCore tab converter with frozen WTF; no WebKit object ABI is used."""
import argparse
import hashlib
import io
import json
...
HELPERS = {
    'Shared/Extensions': ('WebExtensionTabQueryParser.h', 'WebExtensionTabQueryParser.cpp'),
    'WebProcess/Extensions/Bindings': ('JSWebExtensionTabParameters.h', 'JSWebExtensionTabParameters.cpp', 'JSWebExtensionString.h', 'JSWebExtensionString.cpp'),
}
...
def native():
    ...
    report = {'scope': 'native structural tab-query parsing and real JavaScriptCore tab conversion only; no WebExtension, context, controller or browser runtime',
              'source_manifest': manifest, 'native_inputs': snapshot, 'frozen_libraries': hashes,
              'compile_results': [], 'linked_webcore_or_webkit': False}
    objects = []
    for name in ('WebExtensionTabQueryParser.cpp', 'JSWebExtensionTabParameters.cpp', 'JSWebExtensionString.cpp', 'EngineExtensionTabJSONTests.cpp'):
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native tab JSON test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionTabJSONTests.cpp'
    ...
    for name in ('test-engine-extension-tab-json.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-tab-json-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-tab-json-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native tab JSON helper stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-tab-json.py', '--native']),
