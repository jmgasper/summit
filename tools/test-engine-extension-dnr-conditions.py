#!/usr/bin/env python3
"""Run native DNR compound URL-condition checks with real WebCore filter bytecode and frozen JavaScriptCore/WTF."""
import argparse
import hashlib
import io
import json
...
CORE_UNITS = ('CombinedFiltersAlphabet.cpp', 'CombinedURLFilters.cpp', 'DFA.cpp',
              ...
              'DFABytecodeInterpreter.cpp', 'NFAToDFA.cpp', 'NFA.cpp', 'SerializedNFA.cpp', 'URLFilterParser.cpp', 'ContentExtensionRule.cpp', 'ContentExtensionURLConditions.cpp')
...
for name in ('ContentExtensionURLConditions.h',):
    HELPERS[name] = Path('Source/WebCore/contentextensions') / name
    HELPERS['WebCore/' + name] = Path('Source/WebCore/contentextensions') / name
for name in ('ResourceLoadInfo.h', 'WebCore/ResourceLoadInfo.h'):
    HELPERS[name] = Path('Source/WebCore/loader/ResourceLoadInfo.h')
COMPILE_UNITS = ('EngineExtensionDNRConditionsTests.cpp', *CORE_UNITS)
...
def native():
    ...
    report = {'scope': 'native compound URL-condition parser/compiler/filter and actual WebCore NFA/DFA bytecode; no full rule-list parser, action execution, extension context, IPC, database or browser runtime',
    ...
    if 'libWebKit' in dynamic or 'libWebCore' in dynamic:
        raise RuntimeError('DNR condition helper unexpectedly links WebCore or WebKit')
    ...
    try:
        ...
        report['exit'] = None
        report['output'] = 'Native DNR condition test timed out and was terminated; failed run'
...
def host(overlay):
    ...
    test = ROOT / 'tests/EngineExtensionDNRConditionsTests.cpp'
    ...
    for name in ('test-engine-extension-dnr-conditions.py', 'native_crash_log.py'):
    ...
    stage = remote('mktemp -d /boot/home/summit/extension-dnr-conditions-tests.XXXXXXXX', check=True,
                   text=True, capture_output=True).stdout.strip()
    if not stage.startswith('/boot/home/summit/extension-dnr-conditions-tests.') or '/' in stage.removeprefix('/boot/home/summit/'):
    ...
    print('Native DNR condition stage: ' + stage, flush=True)
    with (output / 'native.log').open('w') as log:
        result = remote(shlex.join(['python3.10', stage + '/tools/test-engine-extension-dnr-conditions.py', '--native']),
