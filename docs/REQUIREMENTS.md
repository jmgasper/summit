...
| Chrome extensions | Shared native WebExtension runtime: native package review/installation and manager, fingerprint-bound activation, background/pages, storage/cookie fixtures, and approved startup across actual browser restarts. No Chrome compatibility corpus verified. | CRX import and authentication, Chrome key-based identity, updates, Manifest V2/V3, isolated content scripts, background pages/service workers, `chrome.*` API breadth, permissions, actions/popups/options, request filtering, storage, messaging and extension compatibility corpus. |
