...
| Chrome extensions | Native package review/installation and manager; Chrome manifest key derives the Chromium ID, verified through `chrome.runtime.id`, saved grants/storage and browser restarts. Shared runtime has background/page and storage/cookie fixtures. No Chrome compatibility corpus verified. | CRX import and authentication, mixed-browser identity selection, updates, Manifest V2/V3, isolated content scripts, background pages/service workers, `chrome.*` API breadth, permissions, actions/popups/options, request filtering, storage, messaging and extension compatibility corpus. |
