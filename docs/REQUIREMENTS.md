...
| Chrome extensions | Shared native WebExtension runtime: package preparation, fingerprint-bound activation, background/pages, storage/cookie fixtures, and approved startup across actual browser restarts. No Chrome compatibility corpus verified. | CRX/ZIP install/update, Manifest V2/V3, isolated content scripts, background pages/service workers, `chrome.*` API breadth, permissions, actions/popups/options, request filtering, storage, messaging and extension compatibility corpus. |
| Firefox extensions | Shared WebExtension runtime and ZIP/XPI preparation; `browser.*` storage, cookie and permission fixtures. Approved startup persists across browser restarts. No Firefox add-on corpus verified. | Native XPI install/update UI and authentication, Gecko manifest fields, complete `browser.*` Promise APIs and Firefox semantics, WebRequest and representative real add-on verification. |
