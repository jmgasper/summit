...
| Core API messages | The event API, UI listener bookkeeping and generated C++ event binding now compile with their validated IPC messages. The port API and its generated C++ binding now compile too; native page-port ownership passes 22 checks. Runtime reply callbacks, sender conversion, native message routing and storage bindings remain. See [port scope and evidence](webextensions-native-ports.md). Convert the remaining bindings through `UseCPPAPI`, with `JSONValue`/WTF types and existing callbacks. `JSWebExtensionWrapper.cpp` is now included by `PlatformHaiku.cmake`. Move generated interfaces between CMake lists when their IDL output changes. The namespace exposure, API methods, context messages and message validators must agree. |
